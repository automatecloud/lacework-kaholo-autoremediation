...
We recommend to create an S3 test bucket you can use for testing. By default every S3 bucket will not have Versioning enabled. So every new S3 bucket should generate an event for this Policy. By default the compliance check is done once per day (every 24 hours). You can force to create a compliance check for CIS Benchmarks via the Lacework GUI or the API. Out of the compliance check an Event will be generated can then be used to test the Auto Remediation of this Map.
...
The **LW_S3_16** map currently has the following map design:

<img src="LW_S3_16.png">
...
<img src="LW_S3_16_Trigger.png" width="361" height="810">
...
5. The Variable **Recommendation ID** needs to be configured with Value **LW_S3_16**.
...
This configuration will make sure that this map is only triggered if the **rec_id** of the [Webhook payload](https://support.lacework.com/hc/en-us/articles/360034367393-Webhook) is equal to the **LW_S3_16** Event ID.
...
```
{
    ...
    "rec_id": "LW_S3_16",
    "violationdescription": "Ensure the S3 bucket has versioning enabled",
    "eventuuid": "2e600eb5-8573-48d4-98a1-ac344b98c0e3",
    "reportuuid": "8e8198e7-3d9b-4f7a-9814-17d87b4afa79",
   ...
   "suppressionuuid": "752b756f-4c9d-4d49-b858-b821ca5d4a4d"
}
```
...
1. Create a test S3 bucket that is violating the compliance rule for **LW_S3_16** via the CLI command described in the section [How can i use the Map?](https://github.com/automatecloud/lacework-kaholo-autoremediation/tree/main/maps/compliance/aws/LW_S3_16#how-can-i-use-this-map-for-auto-remediation)
...
1. **rec_id:** This shouldn't be changed. The Policy ID will be shown as part of the slack output messages.
...
export EVENTDESCRIPTION="AWS Account 112233445566 (lacework-test) : LW_S3_16 Ensure the S3 bucket has versioning enabled"
export REC_ID=LW_S3_16
```
...
The Map is using the API call put-bucket-versioning to put the S3 bucket versioning.:
