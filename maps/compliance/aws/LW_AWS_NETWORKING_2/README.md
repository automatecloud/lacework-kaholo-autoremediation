# AutoRemediation for the Lacework Policy LW_AWS_NETWORKING_2

## Description
Network ACLs do not allow unrestricted inbound traffic

A Network ACL acts as a stateless, virtual firewall that controls traffic at the subnet level. The default Network ACL
associated with a VPC allows all inbound and outbound traffic. For security purposes, it is recommended to restrict
inbound Network ACLs.

**Rationale:**
A Network ACL which restricts inbound traffic reduces exposure in the event a security group is misconfigured or applied
to the wrong resource. Restricting access using Network ACLs in conjunction with security groups helps prevent
unauthorized access and protects against malicious activity.
...
2. Click Services.
3. Click VPC.
4. Click Network ACLs.
5. Select the Network ACL to be audited.
6. Select the Inbound Rules tab at the bottom of the screen.
7. Look for any rules that list a port range of ALL, which allow unrestricted access to all ports your associated VPC.
...
The **LW_AWS_NETWORKING_2** map currently has the following map design:

<img src="LW_AWS_NETWORKING_2.png">

### Map trigger

Make sure that the Map Webhook Trigger is configured with the following configuration:

<img src="LW_AWS_NETWORKING_2_Trigger.png" width="269" height="608">
...
5. The Variable **Recommendation ID** needs to be configured with Value **LW_AWS_NETWORKING_2**.
...
This configuration will make sure that this map is only triggered if the **rec_id** of the [Webhook payload](https://support.lacework.com/hc/en-us/articles/360034367393-Webhook) is equal to the **LW_AWS_NETWORKING_2** Event ID.
...
```
{
    ...
    "rec_id": "LW_AWS_NETWORKING_2",
    "violationdescription": "Network ACLs do not allow unrestricted inbound traffic",
    ...
    "IgnoreList":[
        "network-acl/acl-1234567890",
        "network-acl/acl-1234567891"
    ],
    "awsaccountid": "950194951070",
    "reporttype": "AWS_CIS_S3",
    "tagname": "LW_AWS_NETWORKING_2",
    "tagvalue": "suppressed",
...
export EVENTDESCRIPTION="AWS Account 112233445566 (lacework-test) : LW_AWS_NETWORKING_2 Network ACLs do not allow unrestricted inbound traffic
export REC_ID=LW_AWS_NETWORKING_2
```
...
The Map Version 1.0 (24th of January 2022) supports the following:
