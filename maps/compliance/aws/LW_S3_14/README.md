...
This Map supports all three possible encryption options. However the default map only includes the configuration for the **Use Server-Side Encryption with Amazon S3-Managed Keys (SSE-S3)**.

We recommend to create an S3 test bucket. This test bucket will have by default encryption disabled. It will generate an event and alert inside Lacework during the next compliance check. By default the compliance check is done once per day (every 24 hours). You can force to create a compliance check for CIS Benchmarks via the Lacework GUI or the API. Out of the compliance check an Event will be generated can then be used to test the Auto Remediation of this Map.

You can also force to disable the bucket encryption by using the following AWS CLI command:
```
aws s3api delete-bucket-encryption --bucket <YOURBUCKETNAME>
```

**Note:** This if for testing of this map only and it should not have any important data.

If you want to know more about the aws s3api delete-bucket-encryption command or want to replace it with a different option  we recommend to take a look at the official documentation available [here](https://awscli.amazonaws.com/v2/documentation/api/latest/reference/s3api/delete-bucket-encryption.html).
...
The **LW_S3_14** map currently has the following map design:

<img src="LW_S3_14.png" width="618" height="338">
...
* The map will trigger the **Print S3 Buckets to enable Encryption via CLI** CommandLine object If you enabled the Auto Remediation inside the LaceworkConfig of the map by using the **"enablebucketencryptionviacli": "true"** setting. It will prepares the target s3 bucket for the logging, print out the name of the S3 buckets that will be remediated, creates a new encryption policy inside the **inputfolder** of the LaceworkConfig. It will use the configuration of **encryptionconfiguration** to create the new JSON file and enables the encryption based on this file.
* The map will trigger the **Apply Encryption Configuration via Object** Amazon-aws-s3 object If you enabled the Auto Remediation inside the LaceworkConfig of the map by using the **"enablebucketencryptionviaobject": "true"** setting.  remediate all S3 buckets using the Method **"Manage Bucket Encryption"** from the [S3 bucket plugin](https://github.com/Kaholo/kaholo-plugin-amazon-s3).
...
<img src="LW_S3_14_Trigger.png" width="264" height="605">
...
5. The Variable **Event ID** needs to be configured with Value **LW_S3_14**.
6. The Variable **Event Severity** needs to be configured with the Value **Any** or **High**

This configuration will make sure that this map is only triggered if the **event_description** of the [Webhook payload](https://support.lacework.com/hc/en-us/articles/360034367393-Webhook) includes the **LW_S3_14** Event ID.
...
The Auto Remediation is disabled if you import the map. It will only be triggered if the configuration **enablebucketencryptionviacli** or **enablebucketencryptionviaobject** of the LaceworkConfig is configured with **true**. Before enabling this we recommend the following:

1. Create a test S3 bucket that is violating the compliance rule for **LW_S3_14** as described in the section [How can i use the Map?](https://github.com/automatecloud/lacework-kaholo-autoremediation/tree/main/maps/compliance/aws/LW_S3_14#how-can-i-use-this-map-for-auto-remediation)
...
5. If you configure the **enablebucketencryptionviacli** settings to **true** to enable the Auto Remediation via CLI, make sure you select an Agent for the Map that has the AWS CLI installed and configured. The Remediation via CLI block will Auto Remediate by first printing the S3 bucket names that will be auto remediated.
...
That new JSON file will be used to configure the encryption of the S3 Bucket via the following AWS CLI command:

```
aws s3api put-bucket-encryption --bucket <YOURBUCKETNAME> --server-side-encryption-configuration file://bucketname-LW_S3_14.json
```
...
6. If you configure the **enablebucketencryptionviaobject** to **true** the Auto Remediation is done by using the [S3 bucket plugin](https://github.com/Kaholo/kaholo-plugin-amazon-s3) using the Method **"Manage Bucket Encryption"** for the S3 bucket itself.
...
export EVENTDESCRIPTION="AWS Account 112233445566 (lacework-test) : LW_S3_14  Ensure all data stored in the S3 bucket is securely encrypted at rest"
```
