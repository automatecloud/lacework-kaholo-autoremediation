...
* The map will trigger the **Print S3 Buckets to enable Encryption via CLI**, **Create Encryption via JSON** and  **Apply Encryption Configuration via CLI** CommandLine object If you enabled the Auto Remediation inside the LaceworkConfig of the map by using the **"dothereplacebucketpolicyremediationviacli": "true"** setting. It will print out the name of the S3 buckets that will be remediated, uses the Kaholo TextEditor Object to create a new Bucket Policy inside the **inputfolder** of the LaceworkConfig and uses the AWS CLI to put the new bucket encryption configuration of the JSON file to the S3 buckets affected.
* The map will trigger the **Apply Encryption Configuration via Object** Amazon-aws-s3 object If you enabled the Auto Remediation inside the LaceworkConfig of the map by using the **"dothedeletebucketpolicyremediationviaobject": "true"** setting. It will remediate all S3 buckets by enable the Encryption of the S3 bucket by using the **"Manage Bucket Encryption"** from the [S3 bucket plugin](https://github.com/Kaholo/kaholo-plugin-amazon-s3).
...
<img src="LW_S3_14_Trigger.png" width="361" height="810">
...
5. The Variable **Recommendation ID** needs to be configured with Value **LW_S3_14**.
...
This configuration will make sure that this map is only triggered if the **rec_id** of the [Webhook payload](https://support.lacework.com/hc/en-us/articles/360034367393-Webhook) is equal to the **LW_S3_14** Event ID.
...
```
{
    ...
    "reportuuid": "0ada522a-6887-447a-962d-83b266faa6b4",
    ...
    "suppressionuuid": "1d794038-3910-43ae-a87d-32f51cd2975e",
...
1. Create a test S3 bucket that is violating the compliance rule for **LW_S3_14** via the CLI command described in the section [How can i use the Map?](https://github.com/automatecloud/lacework-kaholo-autoremediation/tree/main/maps/compliance/aws/LW_S3_14#how-can-i-use-this-map-for-auto-remediation)
...
The Map is using the API call put-bucket-encryption to enable s3 bucket encryption:
