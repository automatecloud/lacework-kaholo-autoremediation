...
We recommend to create a Test S3 Bucket and configure it for Everyone WRITE_ACL permission, so you are generating an Alert inside Lacework during the next compliance check that you can use to test the Auto Remediation.

You can use the following AWS CLI command to configure an existing S3 test bucket with everyone WRITE_ACL permission:
...
The LW_S3_4 map currently looks like the following:

<img src="LW_S3_4.png" width="556" height="395">
...
* It will remediate all the S3 Buckets back to no longer grant 'Everyone' WRITE_ACL permission.
...
<img src="LW_S3_4_Trigger.png" width="272" height="608">
...
5. The Variable **Issue ID** needs to be configured with Value **LW_S3_4**.
6. The Variable **Event Severity** needs to be configured with the Value **Any** or **High**

With that configuration you make sure that this Map is only triggered if within the payload of the Webhook an alarm related to LW_S3_4 Control ID is send.
...
<img src="geteventdetails2.png" width="475" height="360">
...
1. Create a test S3 bucket that is violating the compliance rule for LW_S3_4
...
3. To be even more sure we recommend to configure a suppression setting for the LW_S3_4 compliance check within the Lacework platform to ignore the S3 bucket. Otherwise the ignored Buckets will create additional Events and Alerts within Lacework.
