...
multiple individuals within the organisation; where feasible, phone contact details should
...
listed. If this is unsuccessful and the account behaviour needs urgent mitigation, proactive
measures may be taken, including throttling of traffic between the account exhibiting
suspicious behaviour and the AWS API endpoints and the Internet. This will result in
...
The **AWS_CIS_1_19** map currently has the following map design:

<img src="AWS_CIS_1_19.png">

### Map trigger

Make sure that the Map Webhook Trigger is configured with the following configuration:

<img src="AWS_CIS_1_19_Trigger.png" width="269" height="608">
...
5. The Variable **Recommendation ID** needs to be configured with Value **AWS_CIS_1_19**.
...
This configuration will make sure that this map is only triggered if the **rec_id** of the [Webhook payload](https://support.lacework.com/hc/en-us/articles/360034367393-Webhook) is equal to the **AWS_CIS_1_19** Event ID.
...
```
echo aws maintain current contact detail
```
...
export EVENTDESCRIPTION="AWS Account 112233445566 (lacework-test) : AWS_CIS_1_19 Maintain current contact details
export REC_ID=AWS_CIS_1_19
```
