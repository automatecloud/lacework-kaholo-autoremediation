...
We recommend to create an S3 test bucket and configure it to give all authenticated AWS users READ_ACP permission. This will generate an event and sends an alert inside Lacework during the next compliance check. By default the compliance check is done once per day (every 24 hours). You can initiate to create a compliance check for CIS Benchmarks via the Lacework GUI or the API. When the compliance check is finished and a new violation of the policy was detected an event will be generated that can then be used to test the Auto Remediation of this map via webhook triggers or alerts.
...
5. The Variable **Recommendation ID** needs to be configured with Value **LW_S3_8**.
...
This configuration will make sure that this map is only triggered if the **rec_id** of the [Webhook payload](https://support.lacework.com/hc/en-us/articles/360034367393-Webhook) is equal to the **LW_S3_8** Event ID.
...
```
{
    ...
    "suppressionuuid": "00f75455-db5e-48ff-bd9e-0340a33e9378"
}
```
...
1. Create an test S3 bucket that is violating the compliance rule for **LW_S3_8** via the CLI command described in the section [How can i use the Map?](https://github.com/automatecloud/lacework-kaholo-autoremediation/tree/main/maps/compliance/aws/LW_S3_8#how-can-i-use-this-map-for-auto-remediation)
