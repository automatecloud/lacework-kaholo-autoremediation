...
We recommend to create a Test S3 Bucket and configure it for Everyone FULL_CONTROL permission, so you are generating an Alert inside Lacework during the next compliance check that you can use to test the Auto Remediation.

You can use the following AWS CLI command to configure an existing S3 test bucket with everyone FULL_CONTROL permission:
...
The LW_S3_5 map currently looks like the following:

<img src="LW_S3_5.png" width="624" height="379">
...
* It will remediate all the S3 Buckets back to no longer grant 'Everyone' FULL permission.
...
<img src="LW_S3_5_Trigger.png" width="268" height="607">
...
5. The Variable **Issue ID** needs to be configured with Value **LW_S3_5**.
6. The Variable **Event Severity** needs to be configured with the Value **Any** or **Critical**

With that configuration you make sure that this Map is only triggered if within the payload of the Webhook an alarm related to LW_S3_5 Control ID is send.
...
<img src="geteventdetails2.png" width="472" height="450">
...
1. Create a test S3 bucket that is violating the compliance rule for LW_S3_5
...
3. To be even more sure we recommend to configure a suppression setting for the LW_S3_5 compliance check within the Lacework platform to ignore the S3 bucket. Otherwise the ignored Buckets will create additional Events and Alerts within Lacework.
...
export EVENTDESCRIPTION="AWS Account 112233445566 (lacework-test) : LW_S3_5 Ensure the S3 bucket ACL does not grant 'Everyone' FULL_CONTROL permission [READ, WRITE, READ_ACP, WRITE_ACP]"
```
